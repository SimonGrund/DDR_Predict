...
#Filter all data
d = filter(d_lof, GENE %in% c("APLF"), primaryTumorLocation %in% c("Pancreas", "Esophagus", "Prostate"))
d2 = anti_join(all_samples, d)%>%
  mutate(LOF = 0, GENE = "CDK12", Germline = F, Double = F)
d = bind_rows(d, d2)%>%
  filter( primaryTumorLocation %in% c("Pancreas", "Esophagus", "Prostate"))%>%
...
saveRDS(object = c, file = paste("Figures/Figure_3/CDK12_model|",cp_train$optimal_cutpoint,"|.Rdata" ,sep = ""))
...
#write.table(x = p, file =  "Figures/Figure_4/BRCA_predicted_data.tsv", sep ="\t", col.names = T, row.names = F)

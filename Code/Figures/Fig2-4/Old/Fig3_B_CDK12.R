...
#CDK12
c = readRDS("Figures/Figure_3/CDK12_model.Rdata")

t = as.data.frame(summary(c)$coefficients)
Features = data.frame(Gene = "CDK12", 
                      Feature = rownames(t), 
                      Estimate = t$Estimate,
...
#Make ROC
CDK12 = fread("Figures/Figure_3/CDK12_predicted_data.tsv")
...
t1$"ct" = "Prostate"; t1$AUC = cp$AUC[[1]]
t2 = as.data.frame(cp$roc_curve[[2]])
t2$"ct" = "Skin"; t2$AUC = cp$AUC[[2]]
t3 = as.data.frame(cp$roc_curve[[3]])
t3$"ct" = "Breast"; t3$AUC = cp$AUC[[3]]

tt = bind_rows(t1, bind_rows(t2,t3))
...
ROC = ggplot(tt, aes(x = 1-tnr, y = tpr, color = col))+
  ...
  xlab("1-Specificity")+
  ylab("Sensitivity")+
  scale_color_manual(values = c("cornflowerblue", "brown", "goldenrod"))+
  guides(color = guide_legend(title = ""))
ROC


p = fread("Figures/Figure_3/CDK12_predicted_data.tsv")
...
g2 <<- ggplot(data = p, aes(x = LOF, fill = primaryTumorLocation))+
  ...
  annotate(geom = "text", y = 150, x = cp$optimal_cutpoint[[1]], label = "Prostate cutoff", family = "Arial", size  =2, vjust = 0)+
  geom_vline(xintercept = cp$optimal_cutpoint[[2]], lty = 2, col = "grey")+
  annotate(geom = "text", y = 150, x = cp$optimal_cutpoint[[2]], label = "Skin cutoff", family = "Arial", size  =2, vjust = 0)+
  geom_vline(xintercept = cp$optimal_cutpoint[[3]], lty = 2, col = "grey")+
  annotate(geom = "text", y = 150, x = cp$optimal_cutpoint[[3]], label = "Breast cutoff", family = "Arial", size  =2, vjust = 0)+
  theme(
  ...
  ylab("#Patients")+
  scale_fill_manual(values = c("cornflowerblue", "brown", "goldenrod"))+
...
out = Feature_plot + ROC + g1 + g2 +
  ...
  patchwork::plot_layout(design = layout, guides = "collect") & theme(
    legend.position = "bottom",
    legend.direction = "vertical",
...
ggsave("Figures/Figure_3/CDK12.pdf", device = "pdf", width  = 7.5, height= 5.5)

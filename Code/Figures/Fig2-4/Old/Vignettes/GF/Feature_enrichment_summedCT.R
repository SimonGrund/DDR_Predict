...
#gene = "CDK12"; cohort = c("Ovary", "Breast", "Prostate", "Skin"); feat = c("Signature.13")
 ...
 # # d = filter(d, !Sample_ID %in% brca2$Sample_ID)
  ...
  d = dplyr::select(d, Sample_ID, CDK12, all_of(feat))
...
d3$primaryTumorLocation = paste(cohort, collapse = ", ")
d$primaryTumorLocation = paste(cohort, collapse = ", ")
for(ct in distinct(d, primaryTumorLocation)$primaryTumorLocation){
  x = dplyr::filter(d, CDK12 == "LOF", primaryTumorLocation == ct)%>%dplyr::select(all_of(feat))
  y = dplyr::filter(d, CDK12 == "WT", primaryTumorLocation == ct)%>%dplyr::select(all_of(feat))
...
# 
# #ATRX
# t =feat_enrich(gene = "ATRX",cohort = c("Bone/Soft tissue", "CNS"), feat = "Signature.3")
# g
# t
# 
# #RB1
# t =feat_enrich(gene = "RB1",cohort = c("Urinary tract", "Uterus"), feat =  c("Signature.19", "Signature.7","non.clustered_inv_100Kb.1Mb"))
# g
# t
# 
# #MTOR

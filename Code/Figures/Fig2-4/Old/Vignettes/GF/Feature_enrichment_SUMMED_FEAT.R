...
feat_enrich_summedFeat = function(gene = "CDK12", cohort = c("Ovary", "Breast"), feat = "Signature.13"){
#gene = c("CDK12", "BRCA2"); cohort = c("Ovary", "Breast", "Prostate", "Skin"); feat = c("Signature.13", "Signature.2")
 ...
 # # d = filter(d, !Sample_ID %in% brca2$Sample_ID)
  ...
  d$sum_feats = rowSums(dplyr::select(d, all_of(feat)))
  d = dplyr::select(d, -all_of(feat))
  d2 = pivot_longer(d, cols = sum_feats)
...
for(ct in distinct(d, primaryTumorLocation)$primaryTumorLocation){
    x = dplyr::filter(d, CDK12 == "LOF", primaryTumorLocation == ct)%>%dplyr::select(sum_feats)
    y = dplyr::filter(d, CDK12 == "WT", primaryTumorLocation == ct)%>%dplyr::select(sum_feats)
    if(nrow(y)<3 | nrow(x)<3){next}
    d3$"p"[d3$primaryTumorLocation == ct] = 
      wilcox.test(x = unlist(x[,1]), 
                y = unlist(y[,1]))$p.value
    d3$"n_lOF"[d3$primaryTumorLocation == ct] = nrow(x)
    d3$"n_wt"[d3$primaryTumorLocation == ct] = nrow(y)

}

d3$"Gene"  = paste(gene, collapse = ", ")
d2$"Gene" =  paste(gene, collapse = ", ")
d2 <<- d2
d3
}

#t =feat_enrich(gene = "ARID1A",cohort = c("Unknown"), feat = "non.clustered_inv_100Kb.1Mb")
#cat("WT: ",t$n_wt, "LOF: ", t$n_lOF)

# feat_enrich()
# t =feat_enrich(cohort = c("Ovary", "Breast"), feat = "Signature.13")
# t =feat_enrich(gene = "AR",cohort = c("Ovary", "Breast", "Prostate", "Skin"), feat = "non.clustered_inv_100Kb.1Mb")
# 
...
# 
# # #RB1
#  t =feat_enrich_summedFeat(gene = "RB1",cohort = c("Urinary tract", "Uterus"), feat =  c("Signature.7","Signature.19"))
# t = d2
# 
# t =   t%>%
#   group_by(CDK12)%>%
...
#          CI_up = round(quantile(value, 0.875),0),
#          CI_down = round(quantile(value, 0.125),0))%>%
#   distinct(med, CI_down, CI_up)
# 
# 
#  t =feat_enrich(gene = "RB1",cohort = c("Urinary tract", "Uterus"), feat =  c("non.clustered_inv_100Kb.1Mb"))
# # g
# # t
# # 
# # #MTOR
#  t =feat_enrich_summedFeat(gene = "MTOR",cohort = c("Lung"), feat =c("Signature.4","Signature.18","Signature.22"))
# # g
# # t

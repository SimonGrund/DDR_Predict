...
gene = "ATRX"; cohort = c("CNS", "Bone/Soft tissue"); Cutoff = 25
...
#Per feature
feats = dplyr::select(features, Sample_ID, Signature.3)
feats = dplyr::distinct(feats, Sample_ID, .keep_all = T)
d4 = left_join(d, feats)


ggplot(d4, aes(x = as.factor(y), y = Signature.3, fill = as.factor(y)))+
  geom_boxplot()+
  ggpubr::stat_compare_means(comparisons = list(c(1,2)))

####
# Evaluate retrained models by testing them across cohorts with sufficient mutated samples
#
####
...
Res = fread("Output/Step2_retrained_models/retrained_models.tsv")
models = list.files("Output/Step2_retrained_models/Models/", full.names = T)
...
p1 = ggplot(Features, aes(x= Estimate, y = Feature))+
  ...
  geom_errorbar(aes(xmin = Estimate - stderror, xmax = Estimate + stderror), size = 0.3, width = 0.2)+
  ...
  ylab("")+
  xlab("GLM coef. estimate")+
  ggtitle(label = paste( gene, " - Trained in ", cohort, " cancer", sep =""))
...
#  tmp_CT$"Signature.33" = 0
  ...
  AUC_tmp$label = paste(AUC_tmp$label, "\nn = ", sum(p$obs == "LOF"), " | ",  sum(p$obs == "WT"),
                    "\nAUC = ", round(cut$AUC,2), sep = "")
...
# AUC$label = paste(AUC$label, "\nn = ", sum(p$obs == "LOF"), " | ",  sum(p$obs == "WT"),
#                   "\nAUC = ", round(AUC$AUC,2), sep = "")
#Make AUC plot
p2 = ggplot()+
  geom_line(data = filter(AUC, AUC>= 0.6492155), aes(x = 1-tnr, y = tpr, color = label), size = 0.5)+ #Update cutoff
  geom_line(data = filter(AUC, AUC < 0.6492155), aes(x = 1-tnr, y = tpr, group = label), color = "grey", size = 0.5)+ #Update cutoff
  theme_bw(base_size = 9, base_family = "Arial")+
  theme(
  ...
  annotate(geom = "text", x = 0.64, y = 0.07, hjust = 0, label = "-- AUC<0.65", col = "grey50", size = 2.5,
           family = "Arial")+
 # scale_color_manual(values = c("Darkblue", "brown"))+
  guides(color = guide_legend(title = "", nrow = 4, byrow = T, override.aes = list(size = 5)))  
p2
inner_list[[2]] = p2
...
AB
"
p = gglist[[1]][[1]] + gglist[[1]][[2]] +
  plot_layout(design = layout) +
  plot_annotation(title = "Figure 2-4 (example)") &
  theme(
...
p
ggsave(plot = BRCA, filename = "Output/Figures/Fig2_4_example.pdf", width = 7, height = 3.2, device = "pdf")


###Below code is for the figures included in the paper, above is adapted for the test data set
...
# "
# CDK12 = 
#   gglist[[2]][[1]] + gglist[[2]][[2]] +
#   gglist[[3]][[1]] + gglist[[3]][[2]] +
#   plot_spacer()+ ###
#   gglist[[10]][[1]] + gglist[[10]][[2]] +
#   gglist[[11]][[1]] + gglist[[11]][[2]] + 
#   plot_spacer()+ #JJ
#   gglist[[4]][[1]] + gglist[[4]][[2]] + #ATRX
#   plot_spacer()+ #NN
#   gglist[[16]][[1]] + gglist[[16]][[2]] + #RB1
#   plot_spacer()+ #PP
#   gglist[[13]][[1]] + gglist[[13]][[2]] + #MTOR
#   plot_layout(design = layout) & 
#   guides(color = guide_legend(title = "", nrow = 3, byrow = T, override.aes = list(size = 5)))&
#   #guides(color = guide_legend(title = "", nrow = 3, byrow = T)) &
#   theme(
#     plot.subtitle = element_text(size = 5),
#     axis.title = element_text(size = 7)
#   )
# 
# CDK12
# ggsave(plot = CDK12, filename = "Figures/Figure_4_new/Others.pdf", width = 8.5, height = 8, device = "pdf")  
# 
# 
...
# 
# rest
# ggsave(plot = rest, filename = "Figures/Figure_4_new/single_cohort.pdf", width = 7, height = 8, device = "pdf")  
# 
# 

...
features$Signature.1 = NULL
  ...
  gene = "CDK12"; cohort = c("Ovary", "Breast", "Prostate", "Skin"); Cutoff = 25
 ...
 # d = filter(d, !Sample_ID %in% brca2$Sample_ID)
  ...
  d = dplyr::select(d, Sample_ID, primaryTumorLocation, CDK12, Signature.3, Signature.8, del.mh, non.clustered_inv_1.10Kb, 
                    non.clustered_inv_10.100Kb, non.clustered_inv_1Mb.10Mb)
  

  d2 = pivot_longer(d, cols = c(Signature.3, Signature.8, del.mh, non.clustered_inv_1.10Kb, 
                                non.clustered_inv_10.100Kb, non.clustered_inv_1Mb.10Mb))

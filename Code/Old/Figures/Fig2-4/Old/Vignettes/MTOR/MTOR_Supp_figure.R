...
RB1 =feat_enrich_summedFeat(gene = "MTOR",cohort = c("Skin", "Lung", "Kidney","Colorectal"), 
                            #feat = c("Signature.4", "Signature.22", "Signature.18")
                            feat = c("del.none")
)
RB1 = d2
...
d2 = d2%>%group_by(CDK12, Gene, primaryTumorLocation)%>%mutate(n = n())%>%ungroup()
...
SV = ggplot(filter(d2, CDK12 != "VUS"), aes(x = CDK12, y = value+0.1))+
  ...
  facet_wrap(primaryTumorLocation~Gene, scales = "free_x", ncol = 4)+
  scale_y_log10(label=scientific_10, limits = c(0.03,max(RB1$value)))+
  xlab("")+
  ylab("#SVs")+
 ...
 geom_text(data = distinct(d2, primaryTumorLocation, n, .keep_all = T), size = 2.5, family = "Arial",
...
t = filter(var, Gene == "MTOR")
n_distinct(t$Sample_ID)
t2 = filter(var, Gene %in% c("XRCC5", "XRCC6", "LIG4","53BP1", "PRKDC", "XRCC4"))
n_distinct(t2$Sample_ID)
t3 = filter(var, Gene %in% c("BRCA2", "BRCA1"))
n_distinct(t3$Sample_ID)

...
#Filter all data
d = filter(d_lof, GENE %in% c("CDK12"), primaryTumorLocation %in% c("Breast", "Prostate", "Skin"))
d2 = anti_join(all_samples, d)%>%
  mutate(LOF = 0, GENE = "CDK12", Germline = F, Double = F)
d = bind_rows(d, d2)%>%
  filter( primaryTumorLocation %in% c("Breast", "Prostate", "Skin"))%>%
...
cp_train$AUC 
c
saveRDS(object = c, file = paste("Figures/Figure_3/CDK12_model.Rdata" ,sep = ""))
...
cutpointr(p, x = LOF, class = obs, pos_class = "LOF",
          ...
          direction = ">=", subgroup = primaryTumorLocation)

write.table(x = p, file =  "Figures/Figure_3/CDK12_predicted_data.tsv", sep ="\t", col.names = T, row.names = F)
